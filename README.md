...
This repo is a take-home deliverable, scoped for **local execution** by a
reviewer (or via `docker compose up` on the same machine). Secrets, network
exposure and auth are handled accordingly:

| What | Status | Notes |
|---|---|---|
| `.env` | gitignored, never committed; `.dockerignore` excludes it from images too | `.env.example` ships with placeholders only |
| Secrets in source / git history | Audited — none. `grep` for `sk-*` / `xai-*` / `sk-or-v1-*` against tracked files **and** full history returns zero | |
| Frontend bundle | Built without `VITE_*` env vars, so no key reaches the browser | The BFF holds the LLM key server-side |
| `/healthz` payload | Returns `{ok, mcp_tools, llm_model, llm_base_url, mcp_url}` — model identifier and base URL are public; no API key, no internal hostnames | |
| Logging | `pino` configured to stderr; no `logger.*` call interpolates a credential | |
| Request size | `express.json({ limit: "1mb" })` on the BFF — DoS by payload bloat is bounded | |
| Tool loop | Capped at `MAX_ITERATIONS = 6` per request — no runaway tool chain | |
| stdout discipline | MCP stdio servers die on any non-JSON-RPC byte; we enforce this with pino → stderr **and** an `eslint no-console: error` rule scoped to `src/mcp/**` | |

### What you'd add before exposing this on the public internet

The brief and the demo target localhost. If you fork this and expose either the
BFF (`/chat`) or the MCP HTTP transport (`/mcp`) on a public origin, the
production hardening I'd layer on:

- **CORS allowlist** on both `/chat` and `/mcp` — today neither sets `Access-Control-Allow-Origin`, so a browser from a different origin gets blocked, but an HTTP client (curl / a server-side fetch) is not. Locking the allowlist makes the intent explicit.
- **Rate limiting** on `/chat` — without it, anyone with the public URL can drain the LLM key. `express-rate-limit` or a reverse-proxy rule.
- **Auth** on `/mcp` — MCP servers are typically reached over stdio or behind a trusted gateway. Public Streamable HTTP needs at least a bearer token check.
- **Bind to `127.0.0.1`** by default in non-Docker setups — Node's `listen(port)` binds to all interfaces; that's fine inside a container network, less so on a laptop on shared wifi.
- **Per-request budget** on tool calls — `MAX_ITERATIONS = 6` is a structural cap; in production I'd also track per-IP / per-session spend.

None of these are wired today because they would obscure the deliverable
without adding evaluative signal. They are listed here so the gap is
explicit, not hidden.
